...
const App: React.FC<{ onExit?: () => void }> = ({ onExit }) => {
  ...
  return (
    <div className="fixed inset-0 bg-gray-900 flex flex-col overflow-hidden">
      {/* 頂部導覽列：Glassmorphism */}
      <div className="absolute top-0 left-0 right-0 bg-white/70 backdrop-blur-xl border-b border-white/40 px-4 py-3 shadow-sm z-20 flex items-center justify-between shrink-0 transition-all">
        <div className="flex flex-col min-w-0">
          <span className="text-xs text-slate-500 font-bold truncate max-w-[150px] tracking-wide">
            {projectInfo.name}
          </span>
          <div className="relative inline-flex items-center gap-2">
            <div className="relative inline-flex items-center group">
              <select
                ...
                className="font-bold text-lg bg-transparent pr-6 outline-none appearance-none truncate max-w-[200px] text-slate-800 group-hover:text-orange-600 transition-colors cursor-pointer"
              >
              ...
              <ChevronDown className="absolute right-0 w-4 h-4 pointer-events-none text-slate-400 group-hover:text-orange-500 transition-colors" />
            </div>

            <button
              onClick={() => setShowAddPlanModal(true)}
              className="text-orange-500 hover:text-orange-600 transition p-1 bg-orange-50 hover:bg-orange-100 rounded-full"
              title="新增平面圖"
            >
              <PlusCircle size={22} />
            </button>
        ...
        <div className="flex items-center gap-2">
          <button
             ...
             className="p-2.5 rounded-full bg-blue-50/80 text-blue-600 hover:bg-blue-100 hover:text-blue-700 border border-blue-100 backdrop-blur-sm transition-all active:scale-95 shadow-sm"
          >
            <Save size={20} />
          </button>
          ...
          <button
            ...
            disabled={!isZipLoaded}
            title="匯出照片與報表"
            className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold shadow-sm active:scale-95 transition-all backdrop-blur-sm border ${
              isZipLoaded 
                ? 'bg-emerald-500/90 hover:bg-emerald-600 text-white border-emerald-400 shadow-emerald-500/20' 
          ...
          <button
            ...
            title="退出專案"
            className="p-2.5 rounded-full bg-rose-50/80 text-rose-600 hover:bg-rose-100 hover:text-rose-700 border border-rose-100 backdrop-blur-sm transition-all active:scale-95 shadow-sm"
          >
            <LogOut size={20} />
          </button>
        </div>
      ...
      <div
        ref={containerRef}
        className="flex-1 relative overflow-hidden bg-[#2a2a2a] touch-none select-none mt-[64px] mb-[64px]"
      ...
      {/* 底部導覽列：懸浮 Glass Style */}
      <div className="absolute bottom-0 left-0 right-0 bg-white/70 backdrop-blur-xl border-t border-white/40 pb-safe pt-2 px-4 shadow-[0_-4px_20px_-5px_rgba(0,0,0,0.1)] flex justify-around items-center shrink-0 z-20 h-[70px]">
        <button
          onClick={() => setMode('move')}
          className={`flex-1 flex flex-col items-center py-2 rounded-2xl transition-all duration-300 ${
            mode === 'move'
              ? 'text-blue-600 translate-y-[-4px] bg-blue-50/80 shadow-sm'
              : 'text-slate-400 hover:bg-white/40'
          }`}
        >
          <Move className={`mb-1 transition-transform ${mode === 'move' ? 'scale-110' : ''}`} />
          <span className="text-xs font-bold">移動/縮放</span>
        </button>

        <div className="w-px h-8 bg-slate-300/50 mx-4"></div>

        <button
          onClick={() => setMode('mark')}
          className={`flex-1 flex flex-col items-center py-2 rounded-2xl transition-all duration-300 ${
            mode === 'mark'
              ? 'text-red-600 translate-y-[-4px] bg-red-50/80 shadow-sm'
              : 'text-slate-400 hover:bg-white/40'
          }`}
        >
          <MousePointer2
            className={`mb-1 transition-transform ${mode === 'mark' ? 'scale-110' : ''}`}
          />
      ...
      {showExitDialog && (
        <div className="fixed inset-0 bg-slate-900/30 backdrop-blur-md flex items-center justify-center z-[60] px-4 animate-fadeIn">
          <div className="bg-white/80 backdrop-blur-2xl rounded-[2rem] p-6 w-full max-w-sm shadow-2xl border border-white/50 animate-slideUp">
            <h3 className="text-xl font-bold mb-4 text-slate-800 text-center">確認退出</h3>
            <p className="mb-8 text-slate-600 text-center text-sm leading-relaxed font-medium">
              您確定要結束目前的專案嗎？<br />
              <span className="text-red-500">若尚未儲存，所有進度將會遺失。</span>
            </p>
            <div className="flex flex-col gap-3">
              ...
              <button 
                ...
                className="w-full py-3.5 text-slate-400 hover:text-slate-600 transition-colors text-sm font-medium"
              >
                取消
              </button>
            </div>
