...
const App: React.FC<{ onExit?: () => void }> = ({ onExit }) => {
  ...
  // ... (保留 Touch handlers: handleTouchStart, handleTouchMove, handleTouchEnd, updateLoupe)
  ...
  const updateLoupe = (pos: { clientX: number; clientY: number }) => {
    ...
    if (rawX >= 0 && rawX <= imgDimensions.width && rawY >= 0 && rawY <= imgDimensions.height) {
  ...
  // ... (保留 Marker 相關 handlers)
  const handleMarkerClick = (marker: Marker) => {
    setActiveMarker(marker);
    setFormData({ ...marker.data, tempImage: marker.imageBlob });
    setIsModalOpen(true);
  };
  ...
  const openNewMarkerModal = (coord: { x: number; y: number }) => {
    ...
    setActiveMarker({ id: Date.now(), planIndex: currentPlanIndex, x: xPct, y: yPct, seq: nextSeq });
    setFormData((prev) => ({
      ...prev,
