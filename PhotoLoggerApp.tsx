...
import { Plus, Save, LogOut, FileDown, FolderOpen, FilePlus, Flame, Edit, CopyPlus, ArrowLeft } from 'lucide-react';
 ...
 * 施工照片紀錄 APP
 */
const PhotoLoggerApp: React.FC<{ onExit: () => void }> = ({ onExit }) => {
  // --- 狀態管理 ---
  ...
  const getAppBackground = () => {
     if (!project) return "background: linear-gradient(135deg, #e0f2fe 0%, #f0f9ff 50%, #e0e7ff 100%)";
     if (project.type === 'FIRESTOP') return "bg-gradient-to-br from-orange-50 to-red-50"; 
  ...
  if (view === ViewState.HOME) {
    return (
      <div className={`h-screen flex flex-col items-center justify-center p-6 animate-fadeIn relative ${appBgClass}`}>
        ...
        {/* 返回 Launcher 按鈕 */}
        <button onClick={onExit} className="absolute top-6 left-6 p-2 rounded-full bg-white/50 hover:bg-white/80 transition z-50">
        ...
        <div className={`h-screen flex flex-col overflow-hidden relative animate-fadeIn ${appBgClass}`}>
          ...
          <div className="bg-white/60 backdrop-blur-md border-b border-white/40 pt-safe-top pb-3 px-4 flex justify-between items-center shrink-0 z-10 shadow-sm">
            ...
            <div className="flex gap-2">
              <button onClick={handleExportJSON} className="p-2.5 rounded-full bg-emerald-100/50 text-emerald-700 hover:bg-emerald-200/50 border border-emerald-200/50 backdrop-blur-sm transition-all">
                <Save size={20} />
              </button>
              <button onClick={handleExportZip} className="p-2.5 rounded-full bg-blue-100/50 text-blue-700 hover:bg-blue-200/50 border border-blue-200/50 backdrop-blur-sm transition-all">
                 <FileDown size={20} />
              </button>
              <button onClick={() => setShowExitDialog(true)} className="p-2.5 rounded-full bg-rose-100/50 text-rose-700 hover:bg-rose-200/50 border border-rose-200/50 backdrop-blur-sm transition-all">
                <LogOut size={20} />
              </button>
            </div>
          </div>
